...
public class FxWeaver {
    ...
    private static final HashMap<URL, FxControllerAndView> CATCH = new HashMap<>();
    ...
    public FxWeaver(Callback<Class<?>, Object> beanFactory, Runnable closeCommand) {
        ...
        try {
            resourceBundle = getBean(ResourceBundle.class);
        } catch (Exception e) {
            resourceBundle = null;
            e.printStackTrace();
        }
    ...
    public <C, V extends Node> FxControllerAndView<C, V> load(Class<C> controllerClass) {
        return load(controllerClass, resourceBundle);
    }
    ...
    private <C, V extends Node> FxControllerAndView<C, V> loadByView(URL url, ResourceBundle resourceBundle) {
        FxControllerAndView<C, V> catchObj = (FxControllerAndView<C, V>) CATCH.get(url);
        if (catchObj != null) {
            return catchObj;
        }
        FxControllerAndView<C, V> fxControllerAndView = loadByViewUsingFxmlLoader(new FXMLLoader(), url, resourceBundle);
        CATCH.put(url, fxControllerAndView);
